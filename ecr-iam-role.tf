...
data "aws_iam_policy_document" "EC2DescribeInstances" {
  statement {
    effect = "Allow"
    actions = [
...
resource "aws_iam_policy" "EC2DescribeInstances-policy" {
  name   = "${var.app_name}-EC2DescribeInstances-policy"
  policy = data.aws_iam_policy_document.EC2DescribeInstances.json
}
...
resource "aws_iam_role" "my-ecr-iam-role" {
  ...
  # managed_policy_arns = ["arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryFullAccess", "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"]
}
...
resource "aws_iam_role_policy_attachment" "EC2DescribeInstances-policy" {
  role       = aws_iam_role.my-ecr-iam-role.name
  policy_arn = aws_iam_policy.EC2DescribeInstances-policy.arn
}
